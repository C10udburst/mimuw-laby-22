...
/*
    Biblioteka zawiera dodatkowe funkcje trie, które nie są używane w
   ostatecznym programie, ale są przydatne przy testowaniu. Zakładamy, że
   użytkownik zaimplementował trie_value_write_dot()
*/
...
*/
void trie_write_dot_node(FILE* fp, trie_node_t* node, trie_node_t* parent,
                         int index) {
    if (node == NULL) return;
    if (parent != NULL)
    ...
    if (node->extra == NULL)
        ...
        trie_extra_write_dot(node->extra, fp);
    fprintf(fp, "\"]\n");
    for (int i = 0; i < 3; i++) {
